...
// Orders below this amount are pickup-only (no delivery offered)
const MIN_DELIVERY_AMOUNT = 24000
...
export default function MerchantV1Checkout() {
    ...
    const { id, slug } = router.query
      ...
      // Order total (0 while data is still loading)
      const orderAmount = Number(data?.amount || 0)
      // Treat the not-yet-loaded state as "available" so the pickup notice
      // doesn't flash before the order amount arrives.
      const deliveryAvailable = !data || orderAmount >= MIN_DELIVERY_AMOUNT
...
useEffect(() => {
  ...
  // Below the threshold: pickup only, so the total is just the order amount.
  if (amount < MIN_DELIVERY_AMOUNT) {
    setPrice(amount);
    return;
  }

  setPrice(selected.price === '0' ? amount : amount + Number(selected.price));
}, [selected, data]);
    ...
    callPost(`${apiList.merchant}/update`, {
        ...
        phone: values.phone,
        // No delivery method when the order is pickup-only
        delivery: deliveryAvailable ? selected.id : "",
        merchantId: slug
    }).then((res) => {
        setLoading(false)
        if (res?.status) {
              ...
              <form onSubmit={handleSubmit(handlePayment)} className="space-y-8">
                ...
                <div>
                  ...
                  {deliveryAvailable ? (
                    ...
                    <div className="mt-2 p-4 border border-amber-300 bg-amber-50 rounded-lg">
                      <p className="text-sm font-medium text-amber-800">
                        Дэлгүүрээс ирж авна уу
                      </p>
                      <p className="text-xs text-amber-700 mt-1">
                        {formatNumberWithCommas(MIN_DELIVERY_AMOUNT)}₮-с доош дүнтэй захиалгад хүргэлт хийгдэхгүй тул дэлгүүр дээр ирж бараагаа авна уу.
                      </p>
                    </div>
                  )}
                </div>
